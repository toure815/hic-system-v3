...
export function Environment(name: string): BaseURL {
    return `https://${name}-hic-new-backend-vss2.encr.app`
}
...
/**
 * Client is an API client for the hic-new-backend-vss2 Encore application.
 */
...
export namespace auth {
    ...
    export class ServiceClient {
         ...
         */
        public async dbCheck(): Promise<DbCheckResponse> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("GET", `/auth/db-check`)
            return await resp.json() as DbCheckResponse
        }
         ...
         */
        public async syncUser(params: SyncUserRequest): Promise<User> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("POST", `/auth/sync-user`, JSON.stringify(params))
            return await resp.json() as User
        }
    }
}
...
export namespace onboarding {
    ...
    export type OnboardingStep = "identify-provider" | "practice-type" | "specialty" | "business-profile" | "licenses" | "payers" | "required-docs" | "portal-logins"
    ...
    export class ServiceClient {
         ...
         */
        public async completeOnboarding(params: CompleteOnboardingRequest): Promise<OnboardingDraft> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("POST", `/onboarding/complete`, JSON.stringify(params))
            return await resp.json() as OnboardingDraft
        }

        /**
         * Gets the current onboarding draft for the authenticated user.
         */
        public async getDraft(): Promise<GetDraftResponse> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("GET", `/onboarding/draft`)
            return await resp.json() as GetDraftResponse
        }

        /**
         * Saves or updates the onboarding draft for the authenticated user.
         */
        public async saveDraft(params: SaveDraftRequest): Promise<OnboardingDraft> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("POST", `/onboarding/draft`, JSON.stringify(params))
            return await resp.json() as OnboardingDraft
        }
         ...
         */
        public async uploadDocument(params: UploadDocumentRequest): Promise<UploadedDocument> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("POST", `/onboarding/upload`, JSON.stringify(params))
            return await resp.json() as UploadedDocument
        }
    }
}
...
class BaseClient {
    ...
    constructor(baseURL: string, options: ClientOptions) {
        ...
        if (!BROWSER) {
            this.headers["User-Agent"] = "hic-new-backend-vss2-Generated-TS-Client (Encore/v1.51.5)";
        }
